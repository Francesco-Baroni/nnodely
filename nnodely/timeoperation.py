...
class Integrate(Stream, ToStream):
    ...
    """
    @enforce_types
    def __init__(self, output:Stream, *, init:Stream|None = None, method:str = 'euler') -> Stream:
...
class Derivate(Stream, ToStream):
    ...
    """
    @enforce_types
    def __init__(self, output:Stream, input:Stream = None, *, init:Stream|None = None, method:str = 'euler') -> Stream:
        if input is None:
            check(method in SOLVERS, ValueError, f"The method '{method}' is not supported yet")
            solver = SOLVERS[method]()
            output_dt = solver.derivate(output, init = init)
