...
class Model_Parallel(nn.Module):
    ...
    def evaluate_relation(self, args):
        relation, kwargs, available_states, available_inputs, result_dict_shared = args
        layer_inputs = []
        for key in self.relation_inputs[relation]:
            if key in self.all_constants.keys(): ## relation that takes a constant
                layer_inputs.append(self.all_constants[key])
                ...
                layer_inputs.append(result_dict_shared[key])
        output = self.relation_forward[relation](*layer_inputs)
        return (relation, output.detach())
        ...
        while not self.network_outputs.issubset(available_keys): ## i need to climb the relation tree until i get all the outputs
            ...
            for relation in self.relations.keys():
                if (set(self.relation_inputs[relation]).issubset(available_keys)
                    and relation not in available_keys):
                    ready_relations.append(relation)
            if not ready_relations:
                break  # To avoid infinite loop if something goes wrong
            pool_args = [(relation, kwargs, available_states, available_inputs, result_dict) for relation in ready_relations]
            with Pool() as pool:
                results = pool.map(self.evaluate_relation, pool_args)

            ## Save the current relations
            for relation, output in results:
                result_dict[relation] = output
                available_keys.add(relation)
                ## Check if the relations are inside the connect
                for connect_input, connect_rel in self.connect_update.items():
                    if relation == connect_rel:
                        result_dict[connect_input] = connect(kwargs[connect_input], result_dict[relation])
                        available_keys.add(connect_input)
...
class Model_thread(nn.Module):
    def __init__(self, model_def, device):
        super(Model_thread, self).__init__()
        model_def = copy.deepcopy(model_def)
        self.inputs = model_def['Inputs']
        self.outputs = model_def['Outputs']
        self.relations = model_def['Relations']
    ...
    def evaluate_relation(self, relation, kwargs, available_states, available_inputs, result_dict_shared):
        layer_inputs = []
        for key in self.relation_inputs[relation]:
            if key in self.all_constants.keys(): ## relation that takes a constant
                layer_inputs.append(self.all_constants[key])
    ...
    def forward(self, kwargs):
        ...
        while not self.network_outputs.issubset(available_keys): ## i need to climb the relation tree until i get all the outputs
            ...
            if not ready_relations:
                break  # To avoid infinite loop if something goes wrong
            # pool_args = [(relation, self.relation_inputs[relation], kwargs,
            #                 self.all_constants, self.all_parameters,
            #                 available_states, available_inputs, result_dict)
            #                 for relation in ready_relations]
            futures = []
            with ThreadPoolExecutor() as executor:
                for relation in ready_relations:
                    future = executor.submit(self.evaluate_relation, relation, kwargs, available_states, available_inputs, result_dict)
                    futures.append(future)
                
                for future in as_completed(futures):
                    relation, output = future.result()
                    result_dict[relation] = output
                    available_keys.add(relation)
                    ## Check if the relations are inside the connect
                    for connect_input, connect_rel in self.connect_update.items():
...
class Model_jit(nn.Module):
    def __init__(self, model_def, device):
        super(Model_jit, self).__init__()
        model_def = copy.deepcopy(model_def)
        self.inputs = model_def['Inputs']
        self.outputs = model_def['Outputs']
        self.relations = model_def['Relations']
    ...
    def forward(self, kwargs):
        ...
        while not self.network_outputs.issubset(available_keys):
            ready_relations = [
                relation for relation in self.relations
                if (relation not in available_keys and set(self.relation_inputs[relation]).issubset(available_keys))
            ]
            ...
            for relation in ready_relations:
                layer_inputs = []
                for key in self.relation_inputs[relation]:
                    if key in self.all_constants:
                        layer_inputs.append(self.all_constants[key])
                    elif key in available_states:
                        layer_inputs.append(kwargs[key])
                    elif key in available_inputs:
                        layer_inputs.append(kwargs[key])
                    elif key in self.all_parameters:
                        layer_inputs.append(self.all_parameters[key])
                    else:
                        layer_inputs.append(result_dict[key])
                # Fork the computation — autograd supported!
                futures[relation] = torch.jit.fork(self.relation_forward[relation], *layer_inputs)

            for relation, future in futures.items():
